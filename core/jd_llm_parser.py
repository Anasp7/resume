...
def generate_virtual_jd(target_role: str, api_key: str) -> dict:
    ...
    # Use a compact prompt (~200 tokens in, ~300 out = 500 total vs old 1000)
    if api_key:
        compact_prompt = (
        ...
        try:
            llm_result = call_llm_json(
                compact_prompt, api_key, max_tokens=300,
            ...
            if llm_req:
                ...
                logger.info("Virtual JD enhanced by LLM: %d req skills", len(merged_req))
        except Exception as e:
